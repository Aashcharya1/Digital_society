...
import { Hand, User, Footprints } from 'lucide-react';
...
import type { StringState, GameStatus, LineCoordinate, PuppetPart } from '@/lib/definitions';
import { STRINGS, ACTION_STRING_ID, BLOCKING_STRING_ID } from '@/lib/constants';
...
export function KathputliGame() {
  ...
  const resetGame = useCallback(() => {
    ...
    let initialStrings = shuffledStrings.map((s, i) => ({ ...s, slotIndex: i }));

    const actionIndex = initialStrings.findIndex(s => s.id === ACTION_STRING_ID);
    const blockingIndex = initialStrings.findIndex(s => s.id === BLOCKING_STRING_ID);

    // Force a tangle if not already tangled
    if (blockingIndex < actionIndex) {
      const temp = initialStrings[actionIndex];
      initialStrings[actionIndex] = initialStrings[blockingIndex];
      initialStrings[blockingIndex] = temp;
      
      // Re-assign slot indices after swap
      initialStrings = initialStrings.map((s, i) => ({ ...s, slotIndex: i }));
    }
    ...
    for (const string of strings) {
      ...
      if (slotEl && anchorEl) {
        ...
        newCoords.push({
          ...
          color: string.color,
          isAction: string.id === ACTION_STRING_ID,
          isBlocking: string.id === BLOCKING_STRING_ID,
        });
      }
    }
  ...
  const handlePull = (pulledStringId: PuppetPart) => {
    ...
    const actionIndex = strings.find(s => s.id === ACTION_STRING_ID)!.slotIndex;
    const blockingIndex = strings.find(s => s.id === BLOCKING_STRING_ID)!.slotIndex;
    const isUntangled = blockingIndex < actionIndex;

    if (!isUntangled) {
      setGameStatus('lost-tangled');
    } else if (pulledStringId === ACTION_STRING_ID) {
      setGameStatus('won');
      setIsBowing(true);
    } else {
  ...
  const getDialogContent = () => {
    switch(gameStatus) {
      case 'won':
        return {
          title: "Victory!",
          description: "You've untangled the strings! The puppeteer's skill lies in managing complex manipulations to create fluid motion, a testament to their dexterity and artistry."
        };
      case 'lost-tangled':
        return {
          title: "Tangled!",
          description: "The puppet's strings are crossed. A puppeteer must clear the lines before any action."
        };
      case 'lost-wrong':
        return {
          title: "Wrong Move!",
          description: "You pulled the wrong string! Precision is key in puppetry to convey the intended emotion."
        };
      default:
  ...
  return (
    <div className="w-full max-w-lg mx-auto font-body">
      <header className="text-center mb-4">
        <h1 className="text-4xl sm:text-5xl font-headline font-bold text-primary">Kathputli Unwind</h1>
        <p className="text-muted-foreground mt-2 text-lg">Command: "Make the puppet bow."</p>
      </header>
      ...
      <Card className="shadow-lg overflow-hidden">
        <CardContent ref={gameAreaRef} className="p-2 sm:p-4 relative aspect-[4/3] bg-card">
          ...
          <svg className="absolute top-0 left-0 w-full h-full pointer-events-none" aria-hidden="true">
            {lineCoords.sort((a,b) => (a.isAction || a.isBlocking) ? 1 : -1).map(lc => (
              <line
                key={lc.id}
                x1={lc.x1}
                y1={lc.y1}
                ...
                y2={lc.y2}
                stroke={lc.color}
                strokeWidth={lc.isAction || lc.isBlocking ? 3 : 2}
                strokeOpacity={lc.isAction || lc.isBlocking ? 1 : 0.7}
      ...
      <p className="text-center text-sm text-muted-foreground mt-4 px-4">
        Untangle the puppet: drag the anchors so the <span className="font-bold text-foreground">Left Hand</span> string is to the left of the <span className="font-bold text-foreground">Head</span> string.
      </p>
